...
class LunarLander_new(gym.Env, EzPickle):
    metadata = {
    ...
    def reset(self, height_level=0.25,LEG_SPRING_TORQUE=40):
        self._destroy()
        ...
        # terrain
        CHUNKS = 11
        height = self.np_random.uniform(0.25*H*(1-height_level), 0.25*H*(1+3*height_level), size=(CHUNKS+1,))
        ...
        # far_level = np.random.uniform(1.1, 10, 1)[0]
        far_level = 5
        initial_y = VIEWPORT_H/SCALE
        self.lander = self.world.CreateDynamicBody(
            position=(VIEWPORT_W/SCALE/far_level, initial_y),
            angle=0.0,
            fixtures = fixtureDef(
        ...
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                position=(VIEWPORT_W/SCALE/far_level - i*LEG_AWAY/SCALE, initial_y),
                angle=(i * 0.05),
                fixtures=fixtureDef(
    ...
    def step(self, action, MAIN_ENGINE_POWER=13.0,SIDE_ENGINE_POWER = .6, eng_consume=0.3):
        if self.continuous:
        ...
        reward -= m_power*eng_consume  # less fuel spent is better, about -30 for heuristic landing
        reward -= s_power*0.03
...
class LunarLanderContinuous(LunarLander_new):
    continuous = True
